...
@section('breadcrumb')
    {{ Breadcrumbs::render('dosen.mata-kuliah.indikator-kinerja.show', $mata_kuliah->kode, $mata_kuliah->mataKuliahRegister[0]->jenis, $indikator_kinerja->kode) }}
    <h1 class="fw-bold mb-4">{{ $title . ' ' . $indikator_kinerja->kode }}</h1>
@endsection
                ...
                <div class="d-flex flex-column">
                    <div class="d-flex flex-row mb-4">
                        <div class="d-flex flex-column">
                            ...
                            <div class="">{{ $indikator_kinerja->created_at->translatedFormat('d F Y H:i') }}</div>
                        </div>
                        <div class="d-flex flex-column ms-4">
                            <div class="fw-bold">Diperbarui pada</div>
                            <div class="">{{ $indikator_kinerja->updated_at->translatedFormat('d F Y H:i') }}</div>
                        </div>
                    ...
                    <div class="d-flex flex-column mb-4">
                        ...
                        <div class="">{{ $indikator_kinerja->deskripsi }}</div>
                    </div>
                    ...
                    <div class="d-flex flex-column">
                        ...
                        <table class="table table-bordered table-hover">
                            <tbody>
                                @foreach ($indikator_kinerja->mataKuliahRegister as $mkr)
                                    @foreach ($mkr->pivot->tujuanPembelajaran as $tp)
                                        <tr>
                                            <td class="fw-bold text-nowrap">{{ $tp->kode }}</td>
                                            <td>{{ $indikator_kinerja->deskripsi }}</td>
                                        </tr>
                                    @endforeach
                                @endforeach
                            </tbody>
